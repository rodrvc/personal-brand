...
export const CHAT_INSTRUCTIONS = [
  ...
  "If the message queues several corrections, propose only the first one and say in `text` which one comes next. The exception is compose_from_reference: it takes every style request of its message at once, in its texts' restyle, and nothing comes next.",
...
export function chatInput(
  ...
  return JSON.stringify({ document: { title: ctx.doc.title, slides }, template, library: ctx.library, history: recent, message: text, references,
    ...(layoutLines
      ? { layoutLines, today: new Date().toISOString().slice(0, 10) }
      : {}) });
}
