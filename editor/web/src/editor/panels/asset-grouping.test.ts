...
import { ASSET_KIND_LABEL_KEY } from "./asset-grouping.js";
...
for (const kind of ASSET_KINDS) {
  assert.ok(
    Object.prototype.hasOwnProperty.call(ASSET_KIND_LABEL_KEY, kind),
    `ASSET_KIND_LABEL_KEY is missing an entry for server asset kind "${kind}"`,
  );
}

// And the reverse: a label for a kind the server no longer indexes would be a
// dead dropdown entry that `patchAsset` rejects.
for (const key of Object.keys(ASSET_KIND_LABEL_KEY)) {
  assert.ok(
    (ASSET_KINDS as readonly string[]).includes(key),
    `ASSET_KIND_LABEL_KEY has a stale entry "${key}" that is not a server asset kind`,
  );
}
