...
description: >-
  ...
  explicativo con voz", "convertir estas láminas en video", "hazme un video
  explicando esto", "un video con estas diapositivas", "ponle voz a estas
  láminas", "video de capacitación", "video de inducción", or asks for
  ...
  is `generar-reel-semana`). Prefer this skill over `hyperframes` whenever the
  slides already exist as images and must appear unaltered and unanimated: this
  pipeline is ffmpeg over still frames, its timing comes from measured audio,
  and it adds no motion. Route to `hyperframes` only if the deliverable
  actually needs animation.
---
...
Normalize before it reaches the TTS. If the deck was summarized or generated by
a tool, treat its text as a draft, not a script: it was written to be read, not
spoken. Rewrite for the ear.

### 3. Cast the voice — the owner decides

Query the catalog, render 3–4 samples **with the real script text**, hand them
over, and **wait**. Never claim an accent has no voices without querying the
endpoint. Pace is a casting criterion as much as tone: every clip is as long as
its own narration, so a slower voice does not desynchronize anything — it makes
the whole video longer.

Endpoint, filters, a starting catalog of already-queried voices, and a copyable
snippet that generates and concatenates the samples in one pass:
...
**An empty duration is a failure, not a zero.** A truncated or zero-byte MP3
makes `ffprobe` return nothing, and arithmetic on nothing quietly yields the
tail alone — a silent 0.8s clip the concat accepts. Abort on an empty value;
the reference carries the guard.

Check that the slide count and the audio count match before rendering, too. A
loop driven by slides cannot see a script block that has no slide.
...
Normalize in the script file itself, not in a wrapper, so what you read is what
gets spoken. Then **listen to those passages specifically**: the failure is in
the pronunciation, and only ears catch it.

Token shapes and worked examples:
[`references/script-normalization.md`](references/script-normalization.md).
