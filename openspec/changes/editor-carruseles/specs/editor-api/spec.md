...
### Requirement: Secrets
`OPENAI_API_KEY` SHALL be read from the repo root's `.env` or the environment. The server MUST NOT write it to logs nor return it via the API. With no key, the generation API responds 503 with a message naming the variable.
