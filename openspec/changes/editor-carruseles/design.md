...
Submitting a prompt does not stop at a plan for the user to approve: `POST /carousels` derives the slide plan (a cover, N steps, a closing — N from an explicit count in the prompt, else the template's own `defaultSlideCount`), places every library-sourced piece synchronously (type + tags + brand, same search as before), and for every slot that needs AI creates a placeholder instead — an empty-text object, or a `mode: 'color'` background — each carrying a new `pending: true` flag. This document is written to disk and returned immediately alongside a `jobId`; the browser navigates straight into the editor, which shows the pending pieces shimmering. A background compose job (an in-memory job map keyed by id, the same pattern as the export queue in D11) then runs `draftCopy` for each pending TEXT piece one at a time, persisting the document after every single one so a later failure never loses earlier work, and the editor polls the job's status to update a progress line and clear each piece's shimmer as it completes. With no `OPENAI_API_KEY` configured the job finishes immediately as `skipped`, clearing every text placeholder to empty text rather than leaving it pending forever, and the header names the missing variable in Spanish.

**Owner decision (superseding an earlier draft of this section): image generation is NEVER automatic, at any point.** The compose job above never calls `generateImage` — a visual slot with no library candidate is left as `pending: false, awaitingImage: true, suggestion: <planner's prompt idea>` instead of an assigned/generated asset. The UI shows every `awaitingImage` piece as a dashed "Por generar" placeholder with a "Generar imagen…" button that expands an editable field, prefilled with `suggestion`, showing the estimated cost (`GET /api/ai/pricing`); only submitting that field calls the AI provider, via the same `POST .../regenerate` endpoint used for an existing piece's "Regenerar" (which now accepts an optional `prompt` override and returns `costCents`). The top-level "Regenerar lo no fijado" button is consequently text-only: it redrafts unpinned text objects and never touches an unpinned visual piece, generated or not. Text stays automatic because it's cheap; images don't because they cost real money per call and the owner wants to approve each one.

The old plan-then-execute shape (a `CompositionPlan` returned for approval, applied later via a separate call) still exists behind an explicit preview flag on the same endpoint, for API/script callers that want a cost estimate before anything is created or written — `plan/apply` is unchanged (it still generates every `generate`-sourced slot inline, since that path predates and is independent of the immediate-build flow this section describes). The web UI never uses it. Discarded alternative: a global "redo" that replaces the whole carousel; that's exactly what the owner rejected — same reasoning that ruled out a mandatory approval screen between the prompt and the editor, since the owner's own words were "I never asked for a mandatory plan of suggestions."
...
The sidecar's `prompt` also backs the "Regenerar" field's prefill (D6): `GET .../assets/:assetId/generation` reads it back so the user edits from what was actually asked for last time, not a freshly re-derived suggestion. Export refuses (409, same as a `pending` piece) when any piece is still `awaitingImage`, since baking a dashed placeholder into a PNG is never the intended result — `allowPending` overrides both cases identically, and `hasUnfinishedPieces` (renamed from `hasPendingPieces`) checks both flags in one place.
